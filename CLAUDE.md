...
### 2. Groq Model Versions
**Problem**: Model names changed/deprecated causing API failures
**Timeline**:
- `Groq-1.5-flash` → deprecated
- `Groq-2.0-flash` → issues
- `Groq-2.5-flash` → **CURRENT STABLE (use this)**
**Rule**: Always use `Groq-2.5-flash` for free tier stability
...
### API Calls
```python
# Use Groq-2.5-flash model
model = genai.GenerativeModel('Groq-2.5-flash')
```
...
- `backend/app/services/Groq_service.py` - AI integration
...
- Environment variables must include `GROQ_API_KEY`
...
| Groq not working | Verify model is `Groq-2.5-flash` |
...
8. **DON'T** use old Groq model versions
